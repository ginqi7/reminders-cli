...
extension EKReminder: @retroactive Encodable {
  ...
  public func encode(to encoder: Encoder) throws {
    ...
    try container.encode(self.calendar.title, forKey: .listName)
  ...
  private func formattedDueDate() -> String? {
    ...
    return self.dueDateComponents?.date.map {
      dateFormatter.localizedString(for: $0, relativeTo: Date())
    }
  }

  func toStr() -> String {
    let dateString = self.formattedDueDate().map { " (\($0))" } ?? ""
    let priorityString = Priority(self.mappedPriority).map { " (priority: \($0))" } ?? ""
    let listString = ""
    let notesString = self.notes.map { " (notes: (\($0)))" } ?? ""

    return
      "\(self.title ?? "<unknown>")\(listString)\(notesString)\(dateString)\(priorityString)"
  }
